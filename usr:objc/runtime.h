 ...
 *  associative的主要原理，就是把两个对象相互关联起来，使得其中的一个对象作为另外一个对象的一部分。使用关联，我们可以不用修改类的定义而为其对象增加存储空间。这在我们无法访问到类的源码的时候或者是考虑到二进制兼容性的时候是非常有用。关联是基于关键字的，因此，我们可以为任何对象增加任意多的关联，每个都使用不同的关键字即可。关联是可以保证被关联的对象在关联对象的整个生命周期都是可用的(在垃圾自动回收环境下也不会导致资源不可回收)。
 *
 *
 *
 *
...
typedef OBJC_ENUM(uintptr_t, objc_AssociationPolicy) {
    ...
    //  对应的@property类型是@property(assign)或者@property(unsafe_unretained)，描述是给关联对象指定弱引用
    OBJC_ASSOCIATION_ASSIGN = 0,
    
    //  对应的@property类型是@property(nonatomic , strong)，描述是给关联对象指定非原子的强引用
    OBJC_ASSOCIATION_RETAIN_NONATOMIC = 1,
    
    //  对应的@property类型是@property(nonatomic , copy)，描述是给关联对象指定非原子的copy特性
    OBJC_ASSOCIATION_COPY_NONATOMIC = 3,
    
    //  对应的@property类型是@property(atomic , strong)，描述是给关联对象指定原子的强引用
    OBJC_ASSOCIATION_RETAIN = 01401,
    
    //  对应的@property类型是@property(atomic , copy)，描述是给关联对象指定原子的copy特性
    OBJC_ASSOCIATION_COPY = 01403
    
};

/** 
 * 创建关联:参数设置(源对象，关键字，关联的对象和一个关联策略)
 * 
 * @param object    关联的源对象。
 * @param key        一个 void 指针，对于每个关联，key必须唯一，通常可以使用一个 static variable.
 * @param value     关联的对象。
 ...
 *
 * 断开关联可使用objc_setAssociatedObject(源对象，关键字，关联的对象和一个关联策略) ,关联对象传入nil即可;
 */
